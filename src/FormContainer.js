...
class FormContainer extends React.Component {
  ...
  handleChange(t, element = null) {
    ...
    if (name === "list") {
      ...
      if (exists === undefined) {
        this.setState(
          p => ({ formData: { ...p.formData, list: [...p.formData.list, value] } }),
  ...
  handleFinish() {
    ...
    if (data.length !== 0) {
      if (!data[0].company.length > 0 || !data[0].fname.length > 0 || !data[0].lname.length > 0) return;
    }
  ...
  handleSubmit() {
    ...
    const url = "https://exhibitor.landscapeshow.co.uk/submit.php?type=access";
  ...
  handleDelete(id) {
    this.setState(
      p => ({ formData: { ...p.formData, list: p.formData.list.filter(d => d.id !== id) } }),
      () => {
        this.saveSession(this.state.formData);
      }
    );
  ...
  render() {
    ...
    switch (this.state.currentStep) {
      case 1:
        step = <AccessForm onDelete={this.handleDelete} data={this.state.formData.list} onChange={this.handleChange} onAdd={this.handleAdd} />;
        break;
      case StepEnum.Finish:
        step = <Finish price={this.state.extras} status={this.state.submissionStatus} data={this.state.formData} onChange={this.handleChange} submit={this.handleSubmit} />;
        break;
    ...
    return (
      <div className={style.container}>
        ...
        <div className={style.FormContainer}>
          <div className={style.Sections}>{step}</div>
